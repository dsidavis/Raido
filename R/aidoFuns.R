#
# Todo
#  allow the caller of getOutbreaks to specify the disease by name and we map it an id.
#  get the diseases locally so we don't have to an online request.
#
...
{
    if(!missing(admin))
        return(getLocationAdmin(admin, url = url, curl = curl))
...
function(d, vars = c("id", "name"))
{
    ans = as.data.frame(lapply(vars, function(x) sapply(d, `[[`, x)), stringsAsFactors = FALSE)
    names(ans) = vars
    ans
}
...
function(name, local = TRUE,
         diseases = if(local) getDiseaseDB() else getDiseases(curl = curl),
         curl = getCurlHandle(..., followlocation = TRUE), ...)
{
